...
<section class="account-section login-section">
    <div class="account-area">
        <div class="account-area__inner">
            <h3 class="title text-white text-center">@lang('Please Verify Your Email to Get Access')</h3>
            <h6 class="title text-white text-center">{{auth()->guard('employer')->user()->email}}</h6>
            <form method="POST" action="{{route('employer.verify.email')}}" class="account-form box--border box--shadow p-sm-5 p-3 rounded-3">
                @csrf
                <div class="form-group">
                    <label for="code">@lang('Verification Code') <sup class="text--danger">*</sup></label>
                    <div class="custom-icon-field">
                        <i class="las la-key"></i>
                        <input type="text" id="code" name="email_verified_code" maxlength="7" placeholder="@lang('Enter Verify Code')" class="form--control" required="">
                    </div>
                </div>
                ...
                <p class="mt-3 text-center text-white">
                    ...
                    <a href="{{route('employer.send.verify.code')}}?type=email" class="forget-pass"> @lang('Resend code')</a>
                </p>
