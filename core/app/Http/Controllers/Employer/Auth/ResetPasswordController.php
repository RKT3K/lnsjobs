...
class ResetPasswordController extends Controller
{
    ...
    */
    ...
    public function showResetForm(Request $request, $token = null)
    {
        ...
        if (EmployerPasswordReset::where('token', $token)->where('email', $email)->count() != 1) {
            $notify[] = ['error', 'Invalid token'];
            return redirect()->route('employer.password.request')->withNotify($notify);
        }
        return view(activeTemplate() . 'employer.auth.passwords.reset')->with(
    ...
    public function reset(Request $request)
    {
        ...
        $reset = EmployerPasswordReset::where('token', $request->token)->orderBy('created_at', 'desc')->first();
        if (!$reset) {
