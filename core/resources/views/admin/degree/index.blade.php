...
@section('panel')
                    ...
                    <div class="table-responsive--sm table-responsive">
                        <table class="table table--light style--two">
                            ...
                            @forelse($degrees as $degree)
                                <tr>
                                    <td data-label="@lang('Name')">{{__($degree->name)}}</td>
                                    ...
                                    <td data-label="@lang('Action')">
                                        <a href="javascript:void(0)" class="icon-btn btn--primary ml-1 updateDegree"
                                            data-id="{{$degree->id}}"
    ...
    <div id="degreeModel" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">@lang('Add Degree')</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{route('admin.education.degree.store')}}" method="POST">
                    @csrf
    ...
    <div id="updateDegreeModel" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">@lang('Update Degree')</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{route('admin.education.degree.update')}}" method="POST">
                    @csrf
...
@push('breadcrumb-plugins')
    <a href="javascript:void(0)" class="btn btn-sm btn--primary box--shadow1 text--small addDegree" ><i class="fa fa-fw fa-paper-plane"></i>@lang('Add New Degree')</a>
@endpush
...
<script>
    ...
    $('.addDegree').on('click', function() {
        $('#degreeModel').modal('show');
    });
    
    $('.updateDegree').on('click', function() {
        var modal = $('#updateDegreeModel');
