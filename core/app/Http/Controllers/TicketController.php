...
class TicketController extends Controller
{
    ...
    public function supportTicket()
    {
        ...
        if(Auth::user()){
            $supports = SupportTicket::where('user_id', getUser()->id)->orderBy('priority', 'desc')->orderBy('id','desc')->paginate(getPaginate());
            $view = 'user';
        }
        else if(Auth::guard('employer')->user()){
            $supports = SupportTicket::where('employer_id', getUser()->id)->orderBy('priority', 'desc')->orderBy('id','desc')->paginate(getPaginate());
            $view = 'employer';
        }
        else{
            abort(404);
    ...
    public function viewTicket($ticket)
    {
        ...
        $pageTitle = "Support Tickets";
        if (Auth::user()) {
            $my_ticket = SupportTicket::where('ticket', $ticket)->where('user_id',auth()->id())->orderBy('id','desc')->firstOrFail();
        }elseif(Auth::guard('employer')->user()){
    ...
    public function replyTicket(Request $request, $id)
    {
        if (Auth::user()) {
            ...
            $ticket = SupportTicket::where('employer_id',auth()->guard('employer')->user()->id)->where('id',$id)->firstOrFail();
        }else{
