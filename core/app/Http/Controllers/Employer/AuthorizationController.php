    ...
    public function authorizeForm()
    {
        if (auth()->guard('employer')->user()) {
            ...
            if (!$user->status) {
                ...
                return view($this->activeTemplate.'employer.auth.authorization.email', compact('user', 'pageTitle'));
            }elseif (!$user->sv) {
                if (!$this->checkValidCode($user, $user->ver_code)) {
                    $user->ver_code = verificationCode(6);
                    $user->ver_code_send_at = Carbon::now();
                ...
                return view($this->activeTemplate.'employer.auth.authorization.sms', compact('user', 'pageTitle'));
            }elseif (!$user->tv) {
                $pageTitle = 'Google Authenticator';
                return view($this->activeTemplate.'employer.auth.authorization.2fa', compact('user', 'pageTitle'));
            }else{
