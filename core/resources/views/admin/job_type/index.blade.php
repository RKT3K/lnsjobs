...
@section('panel')
                    ...
                    <div class="table-responsive--sm table-responsive">
                        <table class="table table--light style--two">
                            ...
                            @forelse($jobTypes as $jobType)
                                <tr>
                                    <td data-label="@lang('Name')">{{__($jobType->name)}}</td>
                                    ...
                                    <td data-label="@lang('Action')">
                                        <a href="javascript:void(0)" class="icon-btn btn--primary ml-1 updateJobType"
                                            data-id="{{$jobType->id}}"
    ...
    <div id="jobTypeModel" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{route('admin.job.setting.type.store')}}" method="POST">
                    @csrf
    ...
    <div id="updateJobTypeModel" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">@lang('Update Job Type')</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{route('admin.job.setting.type.update')}}" method="POST">
                    @csrf
...
@push('breadcrumb-plugins')
    <a href="javascript:void(0)" class="btn btn-sm btn--primary box--shadow1 text--small addJobType" ><i class="fa fa-fw fa-paper-plane"></i>@lang('Add New Job Type')</a>
@endpush
...
<script>
    ...
    $('.addJobType').on('click', function() {
        $('#jobTypeModel').modal('show');
    });
    
    $('.updateJobType').on('click', function() {
        var modal = $('#updateJobTypeModel');
